...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if(size_==0){
    ...
    while((cur->next!=nullptr)&&(newData>(cur->data)) ){
      cur=cur->next;
    }
    if((cur->next==nullptr) && (newData>(cur->data))){
      cur->next=newN;
      newN->prev=cur;
      newN->next=nullptr;
      tail_=newN;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // the function.
  LinkedList<T> merged;
  Node * l=left.getHeadPtr(),* r=right.getHeadPtr();
  bool flag_l=(left.empty()),flag_r=right.empty();
  while(!((flag_l)&&(flag_r))){
    
    
    T cur_data;
    if((flag_r)||((!flag_l)&&(l->data<=r->data))){
      cur_data=l->data;
      if(l->next!=nullptr){
        l=l->next;
      }
